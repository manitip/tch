...
class ReportsApiTests(unittest.TestCase):
    ...
    def test_list_report_recipients_excludes_admin_chat_and_subscriber(self):
        fake_settings = {"report_chat_id": 111}
        fake_allow = {
            111: {"active": True, "role": "admin"},
            222: {"active": True, "role": "viewer"},
        }
        fake_rows = [{"telegram_id": 111}, {"telegram_id": 222}]

        with patch.object(app, "refresh_allowlist_if_needed", return_value=fake_allow), \
             patch.object(app, "db_fetchall", return_value=fake_rows):
            recipients = app.list_report_recipients(fake_settings)

        self.assertEqual(recipients, [222])
