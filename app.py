...
def api_admin_full_system_test(
    u: sqlite3.Row = Depends(require_role("admin")),
):
    checks: List[Dict[str, Any]] = []

    def add_check(name: str, fn):
        try:
            details = fn()
    ...
    def check_db():
        row = db_fetchone("SELECT 1 AS v;")
        return {"status": "ok" if row and int(row["v"]) == 1 else "fail", "db_path": CFG.DB_PATH}

    def check_settings_row():
        s = get_settings()
        chat_id = s["report_chat_id"]
        return {
            "status": "ok",
            "timezone": s["timezone"],
            "report_chat_configured": bool(chat_id),
            "daily_expenses_enabled": bool(int(s["daily_expenses_enabled"] or 0)),
        }

    def check_scheduler_state():
        jobs = scheduler.get_jobs()
        job_ids = sorted([j.id for j in jobs])
        required = {"job_sunday_report", "job_backup_daily"}
        if bool(int(get_settings()["daily_expenses_enabled"] or 0)):
            required.add("job_daily_expenses")
        missing = sorted(list(required.difference(set(job_ids))))
    ...
    def check_core_data():
        users_count = int(db_fetchone("SELECT COUNT(*) AS c FROM users WHERE active=1;")["c"])
        months_count = int(db_fetchone("SELECT COUNT(*) AS c FROM months;")["c"])
        status = "ok" if users_count > 0 else "warn"
        return {
            "status": status,
    ...
    def check_storage_paths():
        ...
        writable = os.access(BACKUPS_DIR, os.W_OK) and os.access(ATTACHMENTS_DIR, os.W_OK)
        return {
    ...
    add_check("database", check_db)
    add_check("settings", check_settings_row)
    add_check("scheduler", check_scheduler_state)
    add_check("core_data", check_core_data)
    add_check("storage", check_storage_paths)
