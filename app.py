...
def _sanitize_env_value(name: str, raw: str, *, mask: bool = False) -> str:
    if not raw:
        return raw
    cleaned = raw
    if "\\n" in cleaned:
    ...
    if cleaned != raw:
        reported = _mask_value(cleaned) if mask else cleaned
        CONFIG_LOG.warning(
            "%s contained line breaks or escaped newlines; using sanitized value %s",
            name,
            reported,
        )
    return cleaned
...
def _sanitize_bot_token(raw: str) -> str:
    if not raw:
        return raw
    cleaned = raw
    if "\\n" in cleaned:
    ...
    if cleaned != raw:
        CONFIG_LOG.warning(
            "BOT_TOKEN contained line breaks or escaped newlines; using sanitized value %s",
            _mask_value(cleaned),
        )
...
def load_config() -> Config:
    ...
    base_dir = Path(__file__).resolve().parent
    db_path = _sanitize_env_value("DB_PATH", os.getenv("DB_PATH", "db.sqlite3"))
    users_json_path = _sanitize_env_value("USERS_JSON", os.getenv("USERS_JSON", "users.json"))
...
def ensure_user_profile_fields() -> None:
    ...
    if "username" not in cols:
        db_exec("ALTER TABLE users ADD COLUMN username TEXT;")
    ...
    if "is_active" not in cols:
        ...
        if "active" in cols:
            db_exec("UPDATE users SET is_active=active;")
    if "cash_scopes" not in cols:
        db_exec("ALTER TABLE users ADD COLUMN cash_scopes TEXT;")
    if "cash_ops" not in cols:
...
def webapp_slash(request: Request):
...
@APP.get("/cashapp/", include_in_schema=False)
def cashapp_slash(request: Request):
    return cashapp(request)
